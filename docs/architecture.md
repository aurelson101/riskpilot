...
Le backend suit quatre couches : `Domain` pour les règles métier, `Application` pour les cas d’usage, `Infrastructure` pour Doctrine et les services externes, et `Api` pour les DTO et adaptateurs HTTP. Les modules fonctionnels seront découpés verticalement dans ces couches.
