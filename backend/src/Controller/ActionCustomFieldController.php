...
final readonly class ActionCustomFieldController
{
    ...
    {
        return new JsonResponse(array_map($this->serialize(...), $this->em->getRepository(ActionCustomField::class)->findBy(['organization' => $this->currentUser->get()->getOrganization()], ['order' => 'ASC'])));
    }
    ...
    private function serialize(ActionCustomField $item): array
    {
        return ['id' => $item->getId(), 'key' => $item->getKey(), 'label' => $item->getLabel(), 'type' => $item->getType(), 'options' => $item->getOptions(), 'order' => $item->getOrder(), 'visible' => $item->isVisible(), 'required' => $item->isRequired()];
    }
}
